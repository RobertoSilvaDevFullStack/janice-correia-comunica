...
async function fixMissingColumns() {
  ...
  try {
    ...
    const rolesQuery = `
      SELECT rolname FROM pg_roles WHERE rolname IN ('anon', 'authenticated', 'public');
    `;
    
    const roles = await client.query(rolesQuery);
    console.log('Roles encontradas:', roles.rows.map(r => r.rolname));
    
    // Verificar se estamos no ambiente Supabase ou local
    const isSupabase = roles.rows.some(r => r.rolname === 'anon');
    
    if (isSupabase) {
      console.log('\n🔐 Configurando permissões para ambiente Supabase...\n');
      
      // Configurar permissões para ambiente Supabase
      ...
      console.log('\n🔐 Ambiente local detectado - configurando permissões padrão...\n');
      
      // Para ambiente local, apenas garantir que o usuário atual tem acesso
      const allTables = ['testimonials', 'blog_posts', 'leads', 'palestras', 'mentorias'];
      
      for (const table of allTables) {
        console.log(`🔑 Configurando permissões para ${table}...`);
        
        // Grant para public (acesso geral)
