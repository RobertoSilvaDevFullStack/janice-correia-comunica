...
const TestimonialForm = () => {
  ...
  const { data: existingTestimonial } = useTestimonial(id || "");
  ...
  const {
    ...
    TestimonialFormData & { status: "pending" | "approved" | "rejected" }
  >({
  ...
  useEffect(() => {
    if (existingTestimonial) {
      ...
      setValue("status", existingTestimonial.status);
    }
  }, [existingTestimonial, setValue]);

  const onSubmit = async (
    data: TestimonialFormData & { status?: "pending" | "approved" | "rejected" }
  ) => {
    try {
      if (isEditing && existingTestimonial) {
        await updateMutation.mutateAsync({
          ...
          ...data,
          status: data.status || existingTestimonial.status,
        });
                ...
                <div className="flex-1">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {watch("position") || "Cargo"} •{" "}
                      {watch("company") || "Empresa"}
                    </p>
                  </div>
                </div>
              </div>
